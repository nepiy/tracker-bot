...
- Opt-in OpenSea free-mint alerts for public, GTD, and FCFS zero-price stages starting within the next 12 hours
...
- Automatic free-mint discovery and price-change detection cover public, GTD, and FCFS stages returned by OpenSea's official upcoming calendar while they remain in the 12-hour window. The manual directory can also show later upcoming stages and currently active stages surfaced across OpenSea's calendar categories; live stages with `total_supply >= max_supply` are excluded as sold out. Zero price excludes gas; other private, allowlist, and creator-only stages are intentionally excluded unless OpenSea labels them GTD or FCFS. Approximate USD values depend on OpenSea's current token quote and can move after the alert.
